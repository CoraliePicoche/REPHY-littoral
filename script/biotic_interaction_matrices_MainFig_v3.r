...
pdf("article/graphe/biotic_interaction_matrices_MainFig_allin1_v3.pdf",height=24,width=18)
par(mar=c(8.5,6.,6,3),oma=c(0.1,2.5,.5,0.5))
...
for (gg in 1:length(groupe)){
	g=groupe[gg]
        ...
        for (s in 2:length(sp)){
			...
			if(common_scale){
                        plot(0,0,t='n',xlim=c(0.95,(14+0.25)),ylim=c(0.7,14),yaxt="n",xaxt="n",xlab="",ylab="",main=groupe_nice[gg],cex.main=4.5)
			...
			text(1.+15*pm,1+fact*.45,expression("b"["ij"]),cex=3.5)#,srt=90)
			}
			}else{
			...
			grad_sp=seq(1,length(sp),length.out=length(sp))
                        ...
                        text(length(sp)-1+15*pm,length(sp)-2+fact*.45,expression("b"["ij"]),cex=2.25)#,srt=90)
               		}
...
text(1:13,par("usr")[3] - 0.035, labels=llieux,srt = 30, adj = 1,xpd = TRUE,cex=4)
