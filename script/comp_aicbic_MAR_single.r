...
which_NEI0="cov" #NEI can be either a population or a covariate
...
yli1=-00
yli2=2000
pdf("Rapport/graphe/comp_BIC_essai.pdf",width=15)
par(mar=c(3,6,1,1))
plot(0,0,xlim=c(0,length(option_lieu)*10),ylim=c(yli1,yli2),t="n",xlab="",ylab=expression(paste(Delta,"BIC=BIC-BIC(null model, NEI cov)",sep="")),xaxt="n",lwd=2.5,cex.lab=1.75,cex.axis=1.75,cex=2)
	...
	for(n in 1:length(option_NEI)){
		for (m in 1:length(option_model)){
			if(!(option_model[m]==which_model0&&option_NEI[n]==which_NEI0)){
				...
				rect((l-1)*10+mn,0,(l-1)*10+mn+1,diff_BIC,col=colo[mn])
			}
		}
	}
}
