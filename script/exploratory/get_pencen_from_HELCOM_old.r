...
f_atlas=read.csv("/home/cpicoche/Documents/Plankton/data/PEG_BVOL2016.csv",sep=";",header=TRUE)
...
d_min=min(date)
d_max=max(date)
unique_order=as.character(unique(f_atlas$Order))
...
for (l in 1:1){
			...
			for (t in 1:length(ta_4)){
				...
				a=as.character(unique(unlist(lapply(ta_4[[t]],function(x) f_atlas$Order[grep(x,f_atlas$Genus)]))))
				if(length(a)==1){
					final_pencen[t]=a
				}else if(length(a)==0){
			...
			print("search in Class")
                        ...
                        for (t in list_problem){
                                a=as.character(unique(unlist(lapply(ta_4[[t]],function(x) f_atlas$Order[grep(x,f_atlas$Class)]))))
                                if(length(a)==0){
			...
			for (t in list_problem_2){
				a=as.character(unique(unlist(lapply(ta_4[[t]],function(x) f_atlas$Order[grep(x,f_atlas$Order)]))))
				if(length(a)==0){
