...
yli1=-100
yli2=1500
pdf("Rapport/graphe/comp_AICc_essai_common.pdf",width=15)
par(mar=c(3,6,1,1))
...
#for (l in 1:1){
	f0=paste("data/analyse_MAR/",option_lieu[l],"_",which_model0,"_",which_NEI0,"_regular_common_essai.RData",sep="")
	load(f0)
	AICc0=fit_log$AICc
	...
	for(n in 1:length(option_NEI)){
		for (m in 1:length(option_model)){
			if(!(option_model[m]==which_model0&&option_NEI[n]==which_NEI0)){
				...
				f1=paste("data/analyse_MAR/",option_lieu[l],"_",option_model[m],"_",option_NEI[n],"_regular_common_essai.RData",sep="")
				load(f1)
