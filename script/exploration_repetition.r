...
for (s in liste_sp){
	...
	if(l==1){
	plot(dates,log10(tab[,s]),col=colo[l],t="o",pch=16,cex=acex,lwd=alwd,main=liste_sp[s],ylab="",cex.main=acex,xlim=c(xmin,xmax))
	}else{
...
for (c in cov3_tot){
	...
	if(l==1){
	plot(dates_cov,tab_cov[,c],col=colo[l],t="o",pch=16,cex=acex,lwd=alwd,main=cov3_tot[c],ylab="",cex.main=acex,xlim=c(xmin,xmax))
	}else{
	lines(dates_cov,tab_cov[,c],col=colo[l],t="o",pch=16,cex=acex,lwd=alwd)
	}
}
	legend("bottomright",lieu,col=colo,pch=16,lty=1,cex=2)
}
