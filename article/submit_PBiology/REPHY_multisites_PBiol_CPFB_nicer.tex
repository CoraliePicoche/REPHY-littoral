...
networks. This much weaker intergenus competition suggests that niche differentiation (rather than neutrality)
is commonplace in phytoplankton. Furthermore, the fraction of positive net effects between phytoplanktonic
...
%Author summary is clearly not mandatory 
...
related genera (S3~Fig). This led to sparse, modular
...
(S3~Fig). Taxon $j$ (in columns) has an effect illustrated
...
({*}). The community composition is given in S2~Table.
...
only 12\% of them are (-/-), see also S5~Fig. The sign
...
connectance and linkage density are given in SI Text.}
...
the focal species growth rate, S5~Eq) and impact (summed
effect of the focal species onto other species growth rates, S6~Eq). Relations between inter- and intra-genus interactions emerged
...
taxa. As these genera tended to be more abundant (S7~Fig),
...
corresponds to a given region (S1~Fig). The p-value
...
than a mean-variance scaling of interaction coefficients, S6~Fig).
...
observational count data (listed in S3~Table). We found
...
corresponding to each code, is given in S3~Table. Codes
...
we prove in the SI Text that the intra/inter ratio
...
and the Mediterranean Sea; S1~Fig).
...
compared different `interaction scenarios' based on BIC (S3~Fig), which proved to be satisfactory in our previous analyses
...
vulnerability (average effect of other taxa on a focal taxon, S5~Eq) and impact (average effect of a focal taxon on other taxa,
S6~Eq) on both raw and absolute values of the coefficients,
...
is given in S3~Table. Authors usually reported only
...
(S8~Fig, which increases the mean intergroup interaction
...
(S9~Fig). A second caveat is that while we informed
...
of the REPHY program\cite{REPHY_db} by Ifremer, providing invaluable
