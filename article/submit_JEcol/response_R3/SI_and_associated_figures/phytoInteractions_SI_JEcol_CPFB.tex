...
\end{tabular}
\par\end{centering}
\caption{\textbf{Summary of study sites characteristics}, including the
...
at each site.\label{fig:Time-series}}}
\end{figure}
...
that still allowed interactions between taxa. However, in order to be as comprehensive as possible, we also present results for the full/unconstrained matrix in a section below (``Comparison with a full interaction matrix'').
...
only intrataxon (intragenus) interactions), at 10 sites in 4 different regions,
...
(inter). As time series lengths differ between sites/regions, bars of the same color but from different sites should not be compared.\label{fig:Comparison-of-BIC}}
\end{figure}
...
For all coefficients, significance was estimated by parametric bootstrapping with 1000 samples.
...
effect in 80\% of all estimates while salinity had a negative effect for
66\% of them. The sign of significant temperature effects
...
\par\end{centering}
\caption{\textbf{Effects of environmental variables (temperature, TEMP or salinity,
...
We present in the main text ratios between intra and inter-taxa mean interaction strengths, $\kappa=\frac{\text{mean}|b_{ii}|}{\text{mean}|b_{ij,j\neq i}|}$, in several places. These are computed from three sets of intertaxa interaction coefficients:
\begin{itemize}
...
\par\end{centering}
\caption{\textbf{Pairs of coefficients at each study site}. The effect of
...
is weak or null. Beyond the circle, they are competitive, mutualistic or mixed (+/-) links.
Close to the vertical and horizontal lines, one of the coefficient might be essentially zero.
...
the mean and variance of the intra- and inter-taxa coefficients (i.e.,
...
than the mean of absolute values of intertaxa interactions. The intrataxon
interactions' variance was about 4 times higher than the variance of intertaxa interactions (Fig.~\ref{fig:MeanVar}).

\begin{figure}[H]
\begin{centering}
\includegraphics[width=0.99\textwidth]{moy_vs_var_for_all_interactions_pencen_SI}
\par\end{centering}
\caption{\textbf{Relationship between mean and variance of the intra- and inter-taxa
...
with the mean, at 10 sites in 4 regions, with a model allowing interactions
...
raw values of intertaxa interactions (a) or absolute values of the
intertaxa coefficients (b). We did not take the absolute value of intrataxon coefficients since they were all negative.
\label{fig:MeanVar}}
\end{figure}

The intrataxon interaction strength could be related to the mean abundance
of each taxon (i.e., each genus or groups of genera) as the most self-regulated genera were also the least
...
\par\end{centering}
\caption{\textbf{Relationship between abundance and self-regulation} (intragenus
interaction coefficients). Mean abundance is computed for each taxon/genus
at 10 sites in 4 regions and intragenus interaction strengths are
...
and to ($H_{N,k}$) the phytoplanktonic taxon $k$ can be computed
as:
...
over row $k$ and $S$ is the number of taxa in the network. These
...
for each phytoplanktonic taxon. We measured both its average vulnerability score
(mean strength of the interactions that are applied to a taxon, eq.~\ref{eq:vulnerability})
and its average impact score (mean strength of the interactions the taxon applies
to other taxa, eq.~\ref{eq:impact}) in each network.

\begin{equation}
v_{k}=\mathnormal{\frac{1}{\sum_{i=1}^{S} \boldsymbol{1}_{\mathnormal{b_{ki}\neq0}}}}\sum_{i=1}^{S}b_{ki}\label{eq:vulnerability}
\end{equation}

\begin{equation}
e_{k}=\frac{1}{\sum_{i=1}^{S} \boldsymbol{1}_{\mathnormal{b_{ik}\neq0}}}\sum_{i=1}^{S}b_{ik}\label{eq:impact}
\end{equation}

where $\sum_{i=1}^{S} \boldsymbol{1}_{\mathnormal{b_{ki}\neq0}}$ is the number of
...
interactions (observed predation between organisms, for instance). In fact, we add a handful of predator-prey systems (in red) mainly to give a scale to the plot.
...
\par\end{centering}
\caption{\textbf{Ratios of intra- to inter-taxa mean interaction strength in MAR(1)
...
Table \ref{tab:Studies}. Annotations beginning with letters correspond to the present study. \label{fig:Ratio-of-intra-to-intergroup}}
\end{figure}
...
\par\end{centering}
\caption{\textbf{Relationship between interaction sparsity and study design} in
...
\item the modulus of the dominant eigenvalue of $\mathbf{B}$ is strongly correlated to the real part of the leading eigenvalue of $\mathbf{A} =\mathbf{B}-\mathbf{I}$ and $\mathbf{A} = \mathbf{\log(B)}$ ($>0.99$ in both cases), which means that our results are compatible with continuous-time theory (Fig~\ref{fig:comparison_fig} a)
\item there is no apparent relationship between the continuous-time equivalent metric for stability/resilience and complexity (the latter being measured as the number of taxa times the variance of the intertaxa interaction coefficients) (Fig~\ref{fig:comparison_fig} b).
\end{itemize}
...
Let us note $X=\ln(1+\alpha_{11}N_{1}+\alpha_{12}N_{2})$ and $Y=\ln(1+\alpha_{21}N_{1}+\alpha_{22}N_{2})$.
