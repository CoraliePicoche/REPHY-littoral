...
\usepackage{color}
\usepackage{graphicx}
\usepackage{siunitx} % To avoid greek letter in italic
...
\newcommand{\commentfred}[1]{{\bf \footnotesize\color{blue} #1}}
\newcommand{\fredadd}[1]{{\color{blue} #1}}
%\newcommand\freddel{\bgroup \markoverwith \textcolor{blue}{\rule[0.5ex]{2pt}{0.4pt}} \ULon}
\newcommand\freddel{\bgroup\markoverwith{\textcolor{red}{\rule[0.5ex]{2pt}{0.4pt}}}\ULon}
...
\begin{document}
{\large
\emph{Supporting Information} for \textbf{Strong self-regulation and widespread facilitative interactions in phytoplankton communities} -- Picoche C. \& Barraquand F., Journal of Ecology.}

\appendix

\subsection*{Sampling}

Study sites are shown on Fig. \ref{fig:Map} and the characteristics of each site are given in Table \ref{fig:Map};  they are part of the REPHY monitoring network by Ifremer coastal laboratories \citep{REPHY_db}. The mean temperature \fredadd{at} each site mostly depend\freddel{ed} on its latitude, reflecting the climate of the region, while salinity \fredadd{is also influenced by more local factors such as evaporation and terrestrial inputs from rivers.}%in one site could be considered as a proxy for evaporation and terrestrial inputs from rivers.
...
counts. We focus on phytoplanktonic taxa between 20 and 200 \si\micro m,
...
\end{tabular}
\par\end{centering}
\caption{\textbf{Summary of study site\fredadd{s} characteristics}, including the
...
\end{tabular}
\par\end{centering}
\caption{\textbf{Name and composition of the phytoplanktonic groups.} \fredadd{We generally refer to those groups as \emph{taxa}, though we sometimes use \emph{genera} to emphasize that we are above the species level (even though some of those groups are stricto sensu groups of genera)}. This taxonomic grouping is based on \citet{hernandez_farinas_assessing_2015}.\label{tab:Group_definition}}
\end{table}
...
\fredadd{at} each site.\label{fig:Time-series}}}
\end{figure}
...
that still allowed interactions between \freddel{groups}\fredadd{taxa}. However, in order to be as comprehensive as possible, we also present results for the full/unconstrained matrix in a section below (``Comparison with a full interaction matrix'').
...
only \fredadd{intrataxon (intragenus)} interactions), \fredadd{at} 10 sites in 4 different regions,
...
(inter). As \fredadd{time series lengths differ between sites/regions, bars of the same color but from different sites should not be compared.}\label{fig:Comparison-of-BIC}}
\end{figure}
...
\fredadd{For all coefficients, significance was estimated by parametric bootstrapping with 1000 samples.}
...
than salinity, which is logical since it integrates seasonal variation
...
were significant at the 5\% level for 68\% of all estimates,
as opposed to 16\% for salinity effects. Temperature had a positive
effect in 80\% of \freddel{the cases} \fredadd{all estimates} while salinity had a negative effect for
66\% of \freddel{all estimates} \fredadd{them}. The sign of significant temperature effects
...
\subsubsection*{Computation of intra/inter-taxa interaction strength ratios from interaction coefficients}

We present in the main text ratios between intra and inter-taxa \fredadd{mean} interaction strength\fredadd{s}, $\kappa=\frac{\text{mean}|b_{ii}|}{\text{mean}|b_{ij,j\neq i}|}$, in several places. These are computed from three sets of inter\fredadd{taxa} interaction coefficients:
\begin{itemize}
 \item $b_{ij}^*$, coefficients significantly different from 0 at the 5\% level
 \item $b_{ij}^{\bar{*}}$, coefficients not significantly different from 0
 \item $b_{ij}^0$, coefficients set to 0 before the estimation process.
\end{itemize}
Some coefficients are indeed set to 0 before the estimation process, in order to reduce the dimensionality of the system (see section above). There are therefore different estimates of $\kappa$, depending on the set of $b_{ij}$ chosen for the denominator.
\begin{itemize}
 \item $\{b_{ij}\}=\{b_{ij}^*\} \cup \{b_{ij}^{\bar{*}}\}$, leading to an observed ratio $\kappa$ between 6 and 10. 
 \item $\{b_{ij}\}=\{b_{ij}^*\} \cup \{b_{ij}^{\bar{*}}\} \cup \{b_{ij}^0\}$, leading to a ratio $\kappa$ between 21 and 43. 
 \item $\{b_{ij}\}=\{b_{ij}^*\} \cup \{b_{ij}^{\bar{*}}\} \cup \{b_{ij}^0\}$ where $b_{ij}^{\bar{*}}:=0$ for comparisons with literature-based MAR(1) coefficients, as results in the literature rarely differentiate between non-significant coefficients and coefficients that were set to 0 beforehand. This corresponds to Fig. 4 in the main text. 
 \item $\{b_{ij}\}=\{b_{ij}^*\}$, illustrated in Fig. S9. 
 \end{itemize}
Unless otherwise stated, all analyses presented in the main text and below are based on $\{b_{ij}\}=\{b_{ij}^*\} \cup \{b_{ij}^{\bar{*}}\}$.
...
\begin{figure}[H]
\begin{centering}
\includegraphics[width=0.694\textwidth]{pair_coefficients}
\par\end{centering}
\caption{\textbf{Pairs of coefficients \fredadd{at} each study site}. The effect of
...
is weak or null. \fredadd{Beyond the circle, they are} \fredadd{competitive}, mutualistic or mixed (+/-) links.
\fredadd{Close to the vertical and horizontal lines, one of the coefficient might be essentially zero.}
\label{fig:Pairs-of-coefficients}}
%Within the circle, (+/+), (+/-) or (-/+) interactions can translate in fact into commensalism or amensalism. -> But what about those close to the bars? 
\end{figure}
...
the mean and variance of the intra- and inter\fredadd{taxa} coefficients (i.e.,
...
values of intrataxon coefficients was approximately 8 times higher
than the mean of \freddel{the} absolute values of \freddel{the effect of}inter\fredadd{taxa} interactions. The \fredadd{intrataxon}
interactions' variance was about 4 times higher than the variance of \fredadd{intertaxa} interactions (Fig.~\ref{fig:MeanVar}).

\begin{figure}[H]
\begin{centering}
\includegraphics[width=0.99\textwidth]{moy_vs_var_for_all_interactions_pencen_SI}
\par\end{centering}
\caption{\textbf{Relation between mean and variance of the intra- and inter\fredadd{-taxa}
...
with the mean, \fredadd{at} 10 sites in 4 regions, with a model allowing interactions
...
raw values of inter\fredadd{taxa} interactions (a) or absolute values of the
inter\fredadd{taxa} coefficients (b). We did not take the absolute value of \fredadd{intrataxon} coefficients since they were all negative.
\label{fig:MeanVar}}
\end{figure}

The \fredadd{intrataxon} interaction strength could be related to the mean abundance
of each taxon \fredadd{(i.e., each genus or groups of genera)} as the most self-regulated genera were also the least
...
interaction coefficients). Mean abundance is computed for each \fredadd{taxon/genus}
\fredadd{at} \fredadd{10} site\fredadd{s} in 4 regions and intragenus interaction strengths are
...
and to ($H_{N,k}$) the phytoplanktonic \freddel{group}\fredadd{taxon} $k$ can be computed
as:
...
over row $k$ and $S$ is the number of \freddel{species}\fredadd{taxa} in the network. These
...
for each phytoplanktonic \freddel{group}\fredadd{taxa}. We measured both its vulnerability
(mean strength of the interactions that are applied to a \freddel{group}\fredadd{taxon}, eq.~\ref{eq:vulnerability})
and its impact (mean strength of the interactions the \freddel{group}\fredadd{taxon} applies
to other \freddel{groups}\fredadd{taxa}, eq.~\ref{eq:impact}) in each network.
...
interaction\fredadd{s} \freddel{phenomena} (observed predation between organisms, for instance). In fact, we add a handful of predator-prey systems (in red) mainly to give a scale to the plot.
...
including intertaxa coefficients set to zero (see Fig. 4 in the main text),
...
of the set of coefficients which were deemed significant at the 5\% level in the $\mathbf{B}\text{\textendash}\mathbf{I}$ matrix (see the ``Computation of intra/inter-taxa ratios'' section above for details).
...
text. $T$ is the approximate number of sampling dates in each time series.
\label{tab:Studies}}
\end{sidewaystable}

\begin{figure}[H]
\begin{centering}
\includegraphics[width=1\textwidth]{comparaison_ratio_code_nolog_cleaner_ONLY_SIGNIF_withoutBarraquand_et_al_2018}
\par\end{centering}
\caption{\textbf{Ratio of intra-to-inter\fredadd{taxa} interaction strength\fredadd{s} in MAR(1)
...
Table \ref{tab:Studies}. \fredadd{Annotations beginning with letters correspond to the present study.} \label{fig:Ratio-of-intra-to-intergroup}}
\end{figure}

\begin{figure}[H]
\begin{centering}
\includegraphics[width=1\textwidth]{sparsity_vs_others_species2taxa_withoutBarraquand_et_al_2018}
\par\end{centering}
...
The relation between complexity and stability in community models has been debated for decades in theoretical circles \citep{may_will_1972, allesina_stabilitycomplexity_2015}. In theoretical ecology, random matrix theory has been mostly applied to continuous-time interaction models (\citealt{allesina_stabilitycomplexity_2015}, but see \citealt{cohen_stability_1984}). Here, we intend to connect our statistical discrete-time models to the continuous-time models of stability theory. The discrete-time log-linear model writes $\mathbf{x}_{t+1} = \mathbf{B} \mathbf{x}_{t}$ in the main text. This model can only approximate continuous-time, possibly non-linear dynamics. There are at least two ways to relate discrete-time models to continuous-time dynamics. 

The first approach is to linearize continuous-time dynamics ($\text{d}\mathbf{x}=\mathbf{A}\mathbf{x}\text{dt}$ where $\mathbf{A}$ is the continuous-time community matrix) and to integrate the system over time. In this case, the map from one time-step to the \freddel{other}\fredadd{next} can be written $\mathbf{x}_{t+1} = \text{e}^{\mathbf{A}} \mathbf{x}_{t}$. The discrete-time equivalent of the community matrix $\mathbf{A}$ is then $\mathbf{\log(B)}$ where $\text{e}^{\mathbf{A}}$ is a matrix exponential and $\mathbf{\log(B)}$ the reciprocal of $\text{e}^{\mathbf{A}}$ with $\mathbf{B}$ defined as above. 

The second approach is to first integrate a continuous-time model over a time-step and then linearize the system. In this case, the equivalent matrix $\mathbf{A} = \mathbf{B}-\mathbf{I}$ because it describes the effects of densities on population growth rates (by contrast $\mathbf{B}$ describes effects of log-densities at time $t$ \freddel{to}\fredadd{on} log-densities at time $t+1$). The second approach is illustrated in more detail in the next section of the Supporting Information. \\

Moreover, the measure of resilience differs in discrete- and continuous-time models. In discrete-time models, and therefore in this study, resilience is measured as the maximum modulus of the eigenvalues of the community matrix ($\max(|\lambda_B|)$), through the dominant eigenvalue of $\mathbf{B}$. In continuous-time models, resilience is linked to the maximum real part of the eigenvalues ($\max(\text{Re}(\lambda_A))$), \textit{i.e.,} the real part  \fredadd{of} the leading eigenvalue of $\mathbf{A}$. There is therefore a link to be made between these metrics. We present in Fig.~\ref{fig:comparison_fig} the relationship between resilience metrics $\max(|\lambda_B|)$ and $\max(\text{Re}(\lambda_A))$ to make sure that our results in discrete-time are consistent with continuous-time theory.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.99\textwidth]{modB_f_maxReA.pdf}
    \caption{\textbf{Relationship between discrete-time stability metrics and their continuous-time equivalents} in (a); relationship between $\max(\text{Re}(\lambda_A))$ and an index of complexity, that is the variance of the off-diagonal coefficients weighted by the number of \fredadd{taxa} in each community in (b).}
    \label{fig:comparison_fig}
\end{figure}

We see in Fig.~\ref{fig:comparison_fig} that:
\begin{itemize}
\item leading eigenvalues of $\mathbf{A}$ are similar for $\mathbf{A} = \mathbf{B}-\mathbf{I}$ and $\mathbf{A} = \mathbf{\log(B)}$ (the difference in real parts is around 0.04 for values between -0.45 and -0.2). Hence, $\mathbf{B}-\mathbf{I}$ is a simpler approximation of $\mathbf{A}$ (Fig~\ref{fig:comparison_fig} a),
\item the modulus of the dominant eigenvalue of $\mathbf{B}$ is strongly correlated to the real part of \fredadd{the} leading eigenvalue of $\mathbf{A} =\mathbf{B}-\mathbf{I}$ and $\mathbf{A} = \mathbf{\log(B)}$ ($>0.99$ in both cases), which means that our results are compatible with continuous-time theory (Fig~\ref{fig:comparison_fig} a)
\item there is no apparent relationship between the continuous-time equivalent metric for stability/resilience and complexity (the latter being measured as the number of \fredadd{taxa} times the variance of the intertaxa interaction coefficients) (Fig~\ref{fig:comparison_fig} b).
\end{itemize}
The link between stability and complexity that we study here is therefore of a similar nature to that studied in continuous-time stability theory, and the relation between stability and complexity was found to be non-existent using both discrete-time metrics and equivalent metrics mimicking continuous-time. The absence of relationship between complexity and stability \textit{sensu} resilience therefore appears to be genuine. 
...
perfect for $S\geq3$, \citealt{roeger_discrete_2004}). The Beverton-Holt
...
with $S$ species as the Jacobian has a similar form:
...
However, in the main text, we use the ratio $\kappa=\frac{\overline{|(J-I)_{ii}|}}{\overline{|(J-I)_{ij}|}}$, with average quantities in the numerator and the denominator. To build a relationship between $k$ and $\kappa$, we denote $b_{ij}=(J-I)_{ij}$, $N_T=\sum_{i=1}^S N_i$. In this simple diffuse competition case, all $b_{ij}$ have the same negative sign. Therefore,

\begin{equation}
\begin{aligned}
&\kappa&=&\frac{\overline{b_{ii}}}{\overline{b_{ij}}}\\
\text{where }&\overline{b_{ii}}&=&\frac{1}{n}\sum_{i=1}^S b_{ii}\\
\text{and }&\overline{b_{ij}}&=&\frac{1}{n(n-1)}\sum_{i=1}^S\sum_{j=1,j\neq i}^S b_{ij}.\\
\end{aligned}
\label{eq:bbar}
\end{equation}

Using eq.~\ref{eq:JiivsJij}, we obtain

\begin{equation}
\begin{aligned}
\overline{b_{ij}}&=&\frac{1}{kS(S-1)}&\sum_{i=1}^S\sum_{j=1,j\neq i}^S b_{ii}\frac{N_j}{N_i}\\
&=&\frac{1}{kS(S-1)}&\sum_{i=1}^S\frac{b_{ii}}{N_i}\left( \left(\sum_{j=1}^S N_j\right) - N_i\right)\\
&=&\frac{1}{kS(S-1)}&\sum_{i=1}^S\frac{b_{ii}}{N_i}( N_T - N_i )\\
&=&\frac{1}{k(S-1)}&\left(\frac{1}{S}\sum_{i=1}^S b_{ii}\frac{N_T}{N_i} - \frac{1}{S}\sum_{i=1}^Sb_{ii} \right)\\
&=&\frac{1}{k(S-1)}&\left(\frac{1}{S}\sum_{i=1}^S b_{ii}\frac{N_T}{N_i} - \overline{b_{ii}}\right).\\
\end{aligned}
...
\begin{equation}
\begin{aligned}
& \frac{\overline{b_{ij}}}{\overline{b_{ii}}} & = &\frac{1}{\kappa}&=&\frac{1}{k(S-1)}\left(\frac{1}{S}\sum_{i=1}^S\frac{b_{ii}}{\overline{b_{ii}}}\frac{N_T}{N_i} - 1\right).
\end{aligned}
\end{equation}

Thus, even in the simple case of diffuse competition, the ratio intra/inter might change to some degree between MAR(1) and BH competition, as a function of relative abundances. For even communities, the mapping between Lotka-Volterra and MAR(1) interaction strength ratios is good; the combined effect of variance in self-regulation strength and realistic levels of community uneveness may change this. Also, diffuse competition is a simplification: mapping such interaction strength ratios in the general many-species case is a non-trivial endeavour, and further deviations between the two frameworks could be expected (see \citealp{certain_how_2018}, for the two-species case).
