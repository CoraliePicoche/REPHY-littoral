...
\usepackage{helvet}
\usepackage{geometry}
\geometry{verbose,tmargin=2cm,bmargin=2cm,lmargin=2.3cm,rmargin=2.3cm} %biorxiv version
...
\maketitle
\thispagestyle{empty}
%\doublespacing% for the article
%\vspace{3em}
%\lyxaddress{1 -- Integrative and Theoretical Ecology, LabEx COTE, University of Bordeaux, Pessac, France}
%\lyxaddress{2 -- Institute of Mathematics of Bordeaux, CNRS, University of Bordeaux, Talence, France}
%\vspace{3em}
...
to inter-taxa interactions (a measure of niche differentiation), the frequency of negative vs positive interactions,
...
intergenera interaction strengths and abundance. The less common taxa
tend to be more strongly self-regulated and can therefore maintain
in spite of competition with more abundant ones. 
\item We demonstrate that strong niche differentiation, widespread
...
Neutral theory models, assuming that all individuals have equal birth and death rates and exert equal competitive pressure on conspecifics and heterospecifics alike, produce instead a non-equilibrium coexistence maintained by dispersal from a regional pool. They have been proposed as a solution to the puzzle presented by highly diverse communities \citep{hubbell_unified_2001,rosindell2011unified}. 
...
dwarfing interspecific competition in most cases \citep{adler_coexistence_2010,adler_competition_2018}; see also \citet{volkov_inferring_2009}.
...
models \citep{coyte_ecology_2015}, multiple other modelling \citep{gross_positive_2008,qian2020balance}
...
are considered simultaneously \citep{mougi2012diversity,garcia2018effect,qian2020balance}.
...
the results \citep{barraquand_coastal_2018}. We used the MARSS package v3.9
\citep{holmes_marss_2012,holmes_analysis_2014}, in R v3.3.2 \citep{venables_r_2013},
...
on the two-week timescale. Because temperature and salinity, in addition to their direct effects, sum up
seasonal changes in light and hydrology (salinity is inversely
related to freshwater inflow), they represent the two key drivers
...
67\% of their taxa. In order to have comparable models across sites, we keep
the same 2 covariates, water temperature and salinity, that
...
and the average impact (average effect of a focal taxon on other taxa, eq.~S6), 
on both raw and absolute values of the interaction coefficients. Such vulnerability
and impact scores can be related to in-strength and out-strength in the meta-analysis
of \citet{kinlock_meta-analysis_2019}. We then compared these to
the regulation a focal species exerted on itself. Vulnerability computed on raw coefficient values indicates
...
positive or negative?), while vulnerability computed on absolute coefficient values characterises the strength
of all types of interactions on a taxon (i.e., is a taxon strongly affected by the others?). A similar reasoning applies to the impact score. 
...
\caption{\textbf{Interaction matrices estimated at 10 sites along the French
coastline}. The sites are distributed in 4 regions: (a) Brittany, (b) Oléron, (d) Arcachon, from north to south on the Atlantic coast and (e) Mediterranean (see Supporting Information for a map). Taxon $j$ (in columns) has an effect on taxon $i$'s growth rate (in rows) proportional to the bar height, which corresponds to the $\mathbf{B}-\mathbf{I}$ matrix (community composition in Table~S2, most parsimonious interaction scenario presented). The scale for the coefficient values is given at the bottom left of panel (a). Coefficients significantly different from 0 ($\alpha = 5\%$) are marked by asterisks ({*}). The fraction of positive interactions in each matrix is given by points in (c) while the dashed (resp., dotted) line represents the ratio of interactions remaining positive (resp., negative) for all sites of a given region.}
...
The ratio of mean intragenus to intergenera interaction coefficients
...
the ratio rose to 21-43. Therefore, intragenus interactions were on average one order of magnitude stronger than intergenera interactions.
...
of positive interactions or the mean and variance of the interactions between taxa (Fig.~\ref{fig:Stability-community}). There was a slight
...
\begin{figure}[!ht]
%\centering \includegraphics[width=0.9\textwidth]{complexity_stability_MainFig_pencenjustB_with_E_and_SV_with0}
\centering \includegraphics[width=0.8\textwidth]{complexity_stability_MainFig_pencenjustB_with_E_and_SV_with0} %biorxiv
...
matrix $\mathbf{B}$, plotted on the y-axis, indicates stability \emph{sensu} resilience. Complexity metrics are (a) the fraction of positive coefficients, (b) weighted connectance, (c) the mean intertaxa coefficient and finally (d) the variance of intertaxa coefficients, scaled by the number of taxa. 
Off-diagonal coefficient variance is multiplied by the dimension of the network, that is the number of species in the region.
...
rate, a and b) and impact (effects of the focal taxon on others' growth
rates, c and d), as well as self-regulation, are computed for untransformed
...
\citep{james_constructing_2015}. Relations between intra- and inter-taxa interactions
emerged (Fig.~\ref{fig:Vulnerability_impact}): genera that were
more self-regulating also had a higher average vulnerability score. Those two influences are likely to trade-off:
...
\centering \includegraphics[width=0.75\linewidth]{Ratio_function_dim_tmp_withoutBarraquand_et_al_2018} %biorxiv
\caption{\textbf{Ratios of mean intrataxa to mean intertaxa interaction strength in Multivariate
...
systems, giving a lower bound for the intra/inter ratio. Intertaxa
...
the mean intrataxa vs mean intertaxa interaction estimates of previous MAR(1) studies of long-term
...
large niche differences making on average intrataxon competition much larger
in magnitude than intertaxa interactions.
...
into much higher intragenus than intergenera effects on growth rates, together with a high degree of facilitative
net interactions.
...
a intra/inter ratio of 10 seems like a conservative estimate. It is twice that of \citet{adler_competition_2018} 
...
effects, once abiotic trends are accounted for. Between 40 and 70\% of
...
plant facilitation could be more prevalent in the field. At the moment, it
...
we have argued that for phytoplankton, the strong intrataxon density-dependence
...
may well be a genuine absence. We also found that the percentage of mutualistic interactions, that
...
sites. While network structure did not affect
...
more buffered. By which mechanisms this could happen is so far unclear. As a final note on relationships between interaction matrix coefficients, 
we caution that the trends evidenced are all relatively weak: considerable stochasticity still dominates the distribution of interaction matrix coefficients.
...
found in plant community studies \citep{adler_competition_2018}
...
only weakly. The likely predator effects that we
...
\subsubsection*{Data accessibility} The REPHY dataset has already been published \citep{REPHY_db}. All computer codes and datasets needed for analyses are available online in a GitHub repository (\url{https://github.com/CoraliePicoche/REPHY-littoral}) and archived at Zenodo (\url{http://doi.org/10.5281/zenodo.3743249}). 
