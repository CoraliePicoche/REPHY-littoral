...
\begin{document}
\title{{\Large{}Strong self-regulation and widespread facilitative interactions
...
% 
% Number of words in the manuscript: 4745 %4364
% 
...
to the network complexity and genus self-regulation or abundance. 
...
phytoplanktonic taxa (above 50\% of non-zero interactions on average).
...
in spite of competition with abundant ones. 
\item \textit{Synthesis}: We prove that strong niche differentiation, widespread
...
$1\times S$ noise vector which covers both process and observation
...
\citep{certain_how_2018}, which is how these models are used here.
For ease of interpretation of MAR(1) interaction coefficients, we
also prove the correspondence between the magnitude of intra/inter
interaction strength in a MAR(1) model and a multispecies Beverton-Holt
model, i.e., a discrete-time Lotka-Volterra model \citep{cushing_discrete_2004},
...
matrices in each site, to network descriptors such as the interaction
strength distribution (sign, mean and variance) and weighted connectance
\citep{bersier_quantitative_2002}. The maximum modulus is analogous
...
properties \citep{ives_stability_1999}. Weighted connectance is a
...
on raw values of the coefficients.
...
the estimated mean intertaxa interaction strength, Fig. S8). We considered
both.
...
\begin{figure}[!ht]
\centering % \includegraphics[width=15.8cm]{biotic_interaction_matrices_MainFig_allin1_v4}
...
coastline}. The most parsimonious interaction scenario is presented. Taxon $j$ (in columns) has an effect on taxon $i$'s growth rate (in rows) proportional to the bar height, which corresponds to the $\mathbf{B}-\mathbf{I}$ matrix (community composition in Table~S2). The scale for the coefficient values is given at the bottom left of panel
a). 95\% significance of coefficients is marked by asterisks ({*}). The fraction of positive
...
varied between 6 and 10, not counting coefficients set to 0 in the
estimation process. When we included the zeroes in the interaction
matrix in the computation of the intra/inter mean interaction strength,
the ratio rised to 21-43. Therefore, intragenus interactions were
on average one order of magnitude stronger than intergenus interactions.
...
\begin{figure}[!ht]
\centering{}\centering \includegraphics[width=1\textwidth]{complexity_stability_MainFig_pencenjustB_with_E_and_V_with0}
...
matrix $\mathbf{B}$ indicates stability \emph{sensu} resilience.
...
\begin{figure*}[!h]
\centering \includegraphics[width=0.8\linewidth]{Ratio_function_dim_tmp}
...
in Table~S3. Codes beginning with letters correspond to the present
...
95\% threshold; Fig.~S8 is the same figure taking into account only
...
The rather high intra/intertaxa interaction strength ratio \citep{levine_importance_2009}
...
commensurate in a MAR(1) model. The difference in the intra/inter
ratio that we found should therefore lie elsewhere, which requires
some explanation. One could argue that such high intra/inter ratio
arises because we consider the genus as our baseline taxonomic unit,
rather than the species. It is logical that niche differentiation
...
in which phylogenetic closeness decreases competition strength). However,
there are two arguments suggesting that the niche differences found
here extend to the species level. First, phytoplankton species belonging
to different genera are often found to compete in experiments \citep{titman_ecological_1976,tilman_phytoplankton_1982,descamps-julien_stable_2005}.
In the field-based dataset studied here, the same genera that are
...
variance of the interaction matrices) and their stability (measured
by the dominant eigenvalue of the interaction matrix, which quantifies
...
is not an artefact of a wrong model. This absence of direct link between
...
However, this could also be linked to the limited size of our networks,
as random matrix theory relies on asymptotics \citep{allesina_stabilitycomplexity_2015}.
We should also mention that our interaction matrices (discrete-time
model) are not strictly analogous to the ones used in classical random
matrix theory (continuous-time model). Thus the jury is still out
regarding the absence of complexity-resilience relation, though it
...
genera impacts and the less it impacts other genera. We examined whether
...
partitioning. Furthermore, a low self-regulation was correlated with
...
also explain the lesser impact of highly self-regulated species/genus:
...
while it is harder for the least common taxa to have large impacts.
However, it was more difficult to explain the relationship between
...
environment in ways that affects the less common ones, but the reverse
is not true. We caution, however, that the relationships between vulnerability,
...
hard to detect when all species are present. This would require spatial
segregation between phytoplankton species at the scale of interactions,
i.e., at the microscale. At the moment, it is known that the fine-scale
...
or \citealt{murrell_heteromyopia_2003}). Moreover, even if microscale
hydrodynamics generates spatial structure with segregation between
species, it is not clear either that the ``ghost of competition present''
mechanism could work for phytoplankton, because turbulence rather
than organism movement dictates where the phytoplankton patches can
or cannot appear.
...
exchanges. We also want to thank Gyuri Barabás and an anonymous referee
for their constructive comments. This study was supported by the French
ANR through LabEx COTE (ANR-10-LABX-45).
...
CP and FB contributed equally to
