...
\citep{certain_how_2018}, which is how these models are used here. {\color{blue} For ease of interpretation of MAR(1) interaction coefficients, we also highlight how intra- and inter-taxa interaction strengths in a MAR(1) model map to their counterparts in a multispecies Beverton-Holt model, i.e., a discrete-time Lotka-Volterra model \citep{cushing_discrete_2004},
...
the ratio rose to 21-43 (see the Supporting Information for more details on the way intra/inter ratios were computed). Therefore, intragenus interactions were
...
of 4 to 5 by \citet{adler_competition_2018}.  %{\color{blue}, already debated by \cite{detto_bias_2019}}
Additional estimates using the unconstrained interaction matrix yielded ratios between 8 and 11 depending on the site (Table S3 and Fig. S8 in the Supporting Information), but weak interspecific effects are likely to be inflated in the full model. Therefore, 
a intra/inter ratio of 10 seems like a conservative estimate, twice that of \citet{adler_competition_2018} 
{\color{blue} who use a different model, i.e., a continuous-time Lotka-Volterra competition model. Even though we outline the comparison between MAR(1) models and a discrete-time Lotka-Volterra equivalent in the Supporting Information, the relationship between intra/inter ratios in these two models is not trivial, especially when abundances vary greatly between species. To some degree, different intra/inter ratios could be due to different models or different ways of measuring density-dependencies (e.g., high measurement errors due to using proxies of densities for plants can result in bias in interaction coefficient estimates \citealp{detto_bias_2019}).} %{\color{red} The difference in the intra/inter ratio that we found should therefore also lie elsewhere, which requires some explanation. }
