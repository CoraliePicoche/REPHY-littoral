...
the Supporting Information that the intra/inter ratio should remain commensurate in a MAR(1) model {\color{blue} for diffuse competition. Unfortunately, the skew in species abundances is such that the map between Lotka-Volterra and MAR(1) intra/inter ratios is limited for individual coefficients, but averaged over species pairs as we compute it, this ratio should be similar for both frameworks.} The difference in the intra/inter ratio that we found should therefore lie elsewhere, which requires some explanation. 
One could argue that such high intra/inter ratio arises because we consider the genus as our baseline taxonomic unit,
