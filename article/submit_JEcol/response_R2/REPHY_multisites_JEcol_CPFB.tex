...
abundance along the French coastline. {\color{blue}The time series are modelled
using multivariate autoregressive (MAR) models [[Is there a way to avoid repetitions?]}, allowing for interactions
...
of the lowest observed abundance \citep{hampton_coalescence_2006}. {\color{blue}Time series are plotted in Fig. S2. }
...
matrices for each site to network descriptors, such as the interaction
...
properties \citep{ives_stability_1999}. {\color{blue} We numerically checked that the maximum modulus of the eigenvalues in a discrete-time interaction matrix and its continuous-time model counterpart yield similar information in the Supporting Information. We then compared this resilience measure to complexity metrics.} Weighted connectance is a
...
the ratio rose to 21-43. Therefore, intragenus interactions were
...
95\% threshold; Fig.~S9 is the same figure taking into account only
...
set to zero in the estimation process, could appear very high
in light of previous intra/interspecific competition strength estimates
of 4 to 5 by \citet{adler_competition_2018}. Additional estimates using the unconstrained interaction matrix 
yielded ratios between 8 and 11 depending on the site (Table S3 and Fig. S8 in the Supporting Information),  
but weak interspecific effects are likely to be inflated in the full model. Therefore, 
a intra/inter ratio of 10 seems like a conservative estimate, twice that of \citet{adler_competition_2018}. 
Even though their model is a different one, i.e., Lotka-Volterra competition, we prove in
the Supporting Information that the intra/inter ratio should remain commensurate in a MAR(1) model {\color{blue} provided abundances of all taxa are similar}. 
The difference in the intra/inter ratio that we found should therefore lie elsewhere, which requires
...
in which phylogenetic closeness decreases competition strength). {\color{blue} However, taxonomic resolution is unlikely to be the sole explanation for the high intra/inter ratio interaction strength found here, for two reasons.} First, phytoplankton species belonging
...
though the spectral radius (largest modulus) can be tightly related to the real part of the lead eigenvalue. 
Thus the jury is still out regarding the absence of complexity-resilience relation found here, though it
...
hard to detect when all species are present. This mechanism would require spatial
...
but it is quite unclear how microscale hydrodynamics affects multivariate spatial patterns
...
species, the ``ghost of competition present'' mechanism might not work for phytoplankton as in terrestrial plants, because turbulence rather
than active organism movement dictates where the phytoplankton patches can or cannot appear.
...
exchanges. We also want to thank Bérangère Péquin for her constructive comments and Gyuri Barabás for {\color{blue} thoughtful suggestions to connect to continuous-time theory}. This study was supported by the French
