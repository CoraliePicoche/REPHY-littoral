...
abundance along the French coastline. {\color{blue}We do so 
using multivariate autoregressive (MAR) models, which allows to estimate interactions
...
\citep{certain_how_2018}, which is how these models are used here. {\color{red} For ease of interpretation of MAR(1) interaction coefficients, we
...
model, i.e., a discrete-time Lotka-Volterra model \cite{cushing_discrete_2004},
...
{\color{blue} To investigate stability-complexity relationships,} we compared the maximum modulus of the eigenvalues of the pennate/centric
...
of 4 to 5 by \citet{adler_competition_2018}{\color{blue}, already debated by \cite{detto_bias_2019}}. Additional estimates using the unconstrained interaction matrix 
...
{\color{blue} who use a different model, i.e., a continuous-tim Lotka-Volterra with competition model. Even though we outline the comparison between MAR(1) models and a discrete-time Lotka-Volterra equivalent in the Supporting Information, the relationship between intra/inter ratios in these two models is not completely trivial, especially when abundances can differ by several orders of magnitude.} {\color{red} The difference in the intra/inter ratio that we found should therefore also lie elsewhere, which requires some explanation. }
One could argue that such high intra/inter ratio {\color{blue} could also arise} because we consider the genus as our baseline taxonomic unit,
...
exchanges. We also want to thank Bérangère Péquin for her constructive comments and Gyuri Barabás for {\color{blue} several thoughtful suggestions to connect to continuous-time theory}. This study was supported by the French ANR through LabEx COTE (ANR-10-LABX-45).
