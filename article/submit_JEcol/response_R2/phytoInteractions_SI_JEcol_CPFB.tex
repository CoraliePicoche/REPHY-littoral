...
{\color{blue} The relation between complexity and stability in community model has been a debate in coexistence studies for decades \citep{may_will_1972}. In the ecology field, the Random Matrix Theory to which we compare our data-based results has been mostly developed on continuous-time interaction models \citep{allesina_stabilitycomplexity_2015}. Here, we intend to connnect discrete- and continuous-time models. The discrete log-linear model writes $\mathbf{x}_{t+1} = \mathbf{B} \mathbf{x}_{t}$ in the main text. This model can only approximate continuous-time dynamics (the equation $\text{d}\mathbf{x}=\mathbf{A}\mathbf{x}\text{dt}$, where $\mathbf{A}$ is the continuous-time interaction matrix, would be closer) but there are at least two ways to relate these dynamics. 

The first approach is to linearize continuous-time dynamics and integrate the system over time. In this case, the log-abundance evolution from one time-step to the other can be written $\mathbf{x}_{t+1} = \text{e}^{\mathbf{A}} \mathbf{x}_{t}$. In this case, the discrete-time equivalent of the community matrix $\mathbf{A}$ is $\mathbf{\log(B)}$ where $\mathbf{B}$ is the matrix we present in the main text. 

The second approach is to first integrate a continuous-time model over a time-step and then linearize the system. In this case, $\mathbf{A} \approx \mathbf{B}-\mathbf{I}$. \\

Moreover, the measure of resilience differs in discrete- and continuous-time models. In discrete-time models, and therefore in this study, resilience is measured as the maximum modulus of the eigenvalues of the community matrix ($\max(|\lambda_B|)$), also called the dominant eigenvalue. In continuous-time models, resilience is linked to the maximum real part of the eigenvalues ($\max(\text{Re}(\lambda_A))$), also called the leading eigenvalue. There is therefore a link to be made between these metrics. We present here the relation between resilience metrics $\max(|\lambda_B|)$ = f($\max(\text{Re}(\lambda_A))$) (Fig. \ref{fig:comparison_fig}) to make sure our results are robust to the approximations described above. 

\begin{figure}[H]
    \centering
    \includegraphics[width=0.55\textwidth]{modB_f_maxReA_singlepanel.pdf}
    \caption{Relation between the dominant eigenvalue for $\mathbf{B}$ (discrete time, on the y-axis) and leading eigenvalue for $\mathbf{A} = \mathbf{B}-\mathbf{I}$ and $\mathbf{\log(B)}$ (continuous time, on the x-axis)}
    \label{fig:comparison_fig}
\end{figure}

We see in Fig. \ref{fig:comparison_fig} that:
\begin{itemize}
\item leading eigenvalues of $\mathbf{A}s$ are similar  for $\mathbf{A} = \mathbf{B}-\mathbf{I}$ and $\mathbf{A} = \mathbf{\log(B)}$ (the difference is around 0.04 for values between -0.45 and -0.2
\item the dominant eigenvalue of $\mathbf{B}$ is strongly correlated to the leading eigenvalue of $\mathbf{A} =\mathbf{B}-\mathbf{I}$ and $\mathbf{A} = \mathbf{\log(B)}$ ($>0.99$ in both cases)
\end{itemize}

We therefore consider our results on the absence of relation between stability and complexity to be robust to variations in model formulations.
}
...
{\color{blue} In this paper, the maximum ratio between pairs of abundances vary between 1.01 and 439.5, with a right-skewed distribution and median/mean values around 6/24. Thus, the order of magnitude between interaction strengths might change, depending on the species in presence. Moreover, competition is not completely diffuse: $\alpha_{ij}$ and $k$ vary between species and sites. We can only say that the ratio between intraspecific and interspecific interactions vary on average between 1 and 2 orders of magnitude. }
