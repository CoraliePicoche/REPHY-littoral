...
\begin{itemize}
\item leading eigenvalues of $\mathbf{A}s$ are similar  for $\mathbf{A} = \mathbf{B}-\mathbf{I}$ and $\mathbf{A} = \mathbf{\log(B)}$ (the difference is around 0.04 for values between -0.45 and -0.2, Fig. \ref{fig:comparison_fig}), as guessed by the referee. 
