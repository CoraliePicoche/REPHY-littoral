...
that still allowed interactions between groups. {\color{blue}However, in order to be as exhaustive as possible, we present results for the full/unconstrained matrix in a section below (``Comparison with a full interaction matrix'').}
...
{\color{blue}\subsubsection*{Intra/Inter ratios}
We present in the main text the ratio between intra and inter-taxa interaction strength, $\kappa=\frac{\text{mean}|b_{ii}|}{\text{mean}|b_{ij,j\neq i}|}$
  ...
  There are three types of interspecific interaction coefficients:
\begin{itemize}
...
\end{itemize}
Some coefficients are set to 0 before the estimation process to reduce the dimensionality of the system (see section above). There are therefore different estimates of $\kappa$, depending on the set of $b_{ij}$ chosen for the denominator.
\begin{itemize}
 ...
 \item $\{b_{ij}\}=\{b_{ij}^*\} \cup \{b_{ij}^{\bar{*}}\} \cup \{b_{ij}^0\}$ where $b_{ij}^{\bar{*}}=0$ as results in the literature rarely differentiate between non-significant coefficients and coefficients that were set to 0. This corresponds to Fig. 4 in the main text. 
 \item $\{b_{ij}\}=\{b_{ij}^*\}$, illustrated in Fig. S9. 
 \end{itemize}
Unless otherwise stated, all the following analyses are based on $\{b_{ij}^*\} \cup \{b_{ij}^{\bar{*}}\}$.
...
We therefore consider our results on the absence of relation between stability and complexity to be robust to variations in the model framework.
}
