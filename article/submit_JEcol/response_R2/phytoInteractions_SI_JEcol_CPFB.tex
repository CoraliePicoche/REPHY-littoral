...
that still allowed interactions between groups. {\color{blue}However, in order to be as comprehensive as possible, we also present results for the full/unconstrained matrix in a section below (``Comparison with a full interaction matrix'').}
...
{\color{blue}\subsubsection*{Computation of intra/inter-taxa interaction strength ratios from interaction coefficients}
...
\begin{itemize}
 \item $b_{ij}^*$, coefficients significantly different from 0 at the 95\% level
...
interaction phenomena (observed predation between organisms, for instance). In fact, we add a handful of predator-prey systems (in red) mainly to give a scale to the plot.
...
of the set of coefficients which were deemed significant at the 95\% threshold in the $\mathbf{B}\text{\textendash}\mathbf{I}$ matrix (see the ``Computation of intra/inter-taxa ratios'' section above for details).
...
\begin{figure}[H]
    ...
    \caption{Relationship between discrete-time stability metrics and their continuous-time equivalents in (a); relationship between $\max(\text{Re}(\lambda_A))$ and an index of complexity, that is the variance of the off-diagonal coefficients weighted by the number of species in each community in (b).}
...
\item the modulus of the dominant eigenvalue of $\mathbf{B}$ is strongly correlated to the real part of leading eigenvalue of $\mathbf{A} =\mathbf{B}-\mathbf{I}$ and $\mathbf{A} = \mathbf{\log(B)}$ ($>0.99$ in both cases), which means that our results are compatible with continuous-time theory (Fig~\ref{fig:comparison_fig} a)
...
However, in the main text, we use the ratio $\kappa=\frac{\overline{|(J-I)_{ii}|}}{\overline{|(J-I)_{ij}|}}$, with average quantities in the numerator and the denominator. To build a relationship between $k$ and $\kappa$, we take $b_{ij}=(J-I)_{ij}$, $N_T=\sum_{i=1}^S N_i$ and start by assuming that all $b_{ij}$ have the same sign, negative for competition between species). Then,
...
&=&\frac{1}{kS(S-1)}&\sum_{i=1}^S\frac{b_{ii}}{N_i}\sum_{j=1}^S \left( N_j\right) - N_i\\
...
\begin{equation}
\begin{aligned}
& \overline{b_{ij}}&=&\frac{1}{k(S-1)}\left(\frac{1}{S}\sum_{i=1}^S b_{ii}\frac{N_T}{N_i} - \overline{b_{ii}}\right)\\
\Leftrightarrow & \frac{\overline{b_{ij}}}{\overline{b_{ii}}}&=&\frac{1}{k(S-1)}\left(\frac{1}{S}\sum_{i=1}^S\frac{b_{ii}}{\overline{b_{ii}}}\frac{N_T}{N_i} - 1\right)\\
\Leftrightarrow & \frac{1}{\kappa}&=&\frac{1}{k(S-1)}\left(\frac{1}{S}\sum_{i=1}^S\frac{b_{ii}}{\overline{b_{ii}}}\frac{N_T}{N_i} - 1\right).
\end{aligned}
\end{equation}

{\color{blue} Thus, the order of magnitude between interaction strengths might change between MAR(1) and BH competition, depending on the species in presence and their relative contribution to the community. The mapping between Lotka-Volterra and MAR interaction strength ratios is therefore non-trivial in general.}
