...
{\color{blue} There are different ways to connect a discrete log-linear model with continuous-time dynamics. 

\begin{itemize}
 \item Make a linear continuous-time approximation, which is then integrated over time. In this case, the continuous-time equivalent of the community matrix $\mathbf{B}$ is $\mathbf{\log(B)}$. 
 \item Integrate a continuous-time model over a time period, and then linearize it. In this case, the equivalent of $\mathbf{B}$ is $\mathbf{B-I}$.
\end{itemize}

In addition to different formulations, the measure of resilience is not the same in discrete- and continuous-time models. In discrete-time models, and therefore in this study, resilience is measured as the maximum modulus of the eigenvalues of the community matrix ($\max(|\lambda_B|)$), also called the dominant eigenvalue. In continuous-time models, resilience is measured as the maximum real part of the eigenvalues ($\max(\text{Re}(\lambda_A))$ where $A$ is the continuous-time equivalent of the community matrix), also called the leading eigenvalue. There is therefore a link to be made between these metrics. 

We therefore decompose the relationship into two panels: $\max(|\lambda_B|)$ = f($\max(\text{Re}(\lambda_A))$) (Fig. \ref{fig:comparison_fig} a) and $\max(\text{Re}(\lambda_A))$ = f($SV$) (Fig. \ref{fig:comparison_fig} b). 
...
\begin{itemize}
\item leading eigenvalues of A are similar  for $\mathbf{A} = \mathbf{B}-\mathbf{I}$ and $\mathbf{A} = \mathbf{\log(B)}$ (the difference is around 0.04 for values between -0.45 and -0.2, Fig. \ref{fig:comparison_fig} a), as guessed by the referee. 
...
We therefore consider our results to be robust to variations in model formulations.
}
